...
void OLED_Refresh_Gram(void)
{
	...
	{  
		...
		for(n=0;n<128;n++)OLED_WR_Byte(OLED_GRAM[n][i],OLED_DATA);//显然是一列一列现实的 
	}   
}
...
{	
	...
	{			  
		OLED_SCLK=0;
		if(dat&0x80)OLED_SDIN=1;//数据从高位移入，为一就记为1，为0就记为0
		else OLED_SDIN=0;
		OLED_SCLK=1;
		dat<<=1;   
	}				 
...
{      			    
    ...
    {   
        ...
        for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)OLED_DrawPoint(x,y,mode);//如果在字库中的那一列该位为1那么填充进去
			else OLED_DrawPoint(x,y,!mode);//如果在字库中的那一列中该位为0就直接清掉
			temp<<=1;
			y++;
			if((y-y0)==size)//如果列写完了，那么跳出去再取一列
			{
				y=y0;
				x++;
				break;
...
//命令/数据选择端
	...
	OLED_WR_Byte(0xc0,OLED_CMD); //1~255;默认0X7F (亮度设置,越大越亮)
